...
export class AuthService {
  ...
  async getAdminRole(params: { email: string; action: string }) {
    const user = await this.userService.getUserByEmail(params.email);
    if (!user || !params.action)
      throw new HttpException(
        "email isn't exist or not action",
        HttpStatus.BAD_REQUEST
      );
    if (params.action === "add") {
      await this.userService.setRoles("add", user.id);
    }
    if (params.action === "remove") {
      await this.userService.setRoles("remove", user.id);
    }
    return "changed";
  }
  ...
  private async validateUser(userDto: LoginUserDto) {
    try {
      ...
      throw new UnauthorizedException({
        message: "user is not exist or wrong password",
      });
    } catch (e) {
      throw new UnauthorizedException({
        message: "user is not exist or wrong password",
